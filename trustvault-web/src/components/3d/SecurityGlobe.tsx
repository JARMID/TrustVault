  ...
  return (
    <group position={[1.2, 0, 0]}>
      ...
      <points>
        ...
        <pointsMaterial color={isDark ? "#1e3a5f" : "#94a3b8"} size={0.02} transparent opacity={isDark ? 0.6 : 0.8} sizeAttenuation />
      </points>
      ...
      <mesh rotation={[Math.PI / 2.5, 0, 0]}>
        ...
        <meshBasicMaterial color={isDark ? "#00C6AE" : "#3B82F6"} transparent opacity={0.2} side={THREE.DoubleSide} />
      </mesh>
      <mesh rotation={[Math.PI / 1.8, 0.5, 0.3]}>
        <ringGeometry args={[3.3, 3.31, 64]} />
        <meshBasicMaterial color={isDark ? "#00C6AE" : "#3B82F6"} transparent opacity={0.1} side={THREE.DoubleSide} />
      </mesh>
    </group>
  );
};
      ...
      <div style={{
        ...
        ref={(el) => {
          if (el && scrollYProgress) {
            ...
            interface ElementWithUnsub extends HTMLDivElement {
              __unsub?: () => void;
            }
            (el as ElementWithUnsub).__unsub = unsubscribe;
          }
        }}
      >
        <div style={{
          ...
          border: isDark ? '1px solid rgba(0,198,174,0.2)' : '1px solid rgba(59,130,246,0.2)',
          borderRadius: '12px',
          padding: '16px 20px',
          minWidth: '200px',
          boxShadow: isDark ? '0 8px 32px rgba(0,0,0,0.4)' : '0 8px 32px rgba(0,0,0,0.05)',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', paddingBottom: '8px', borderBottom: isDark ? '1px solid rgba(255,255,255,0.06)' : '1px solid rgba(0,0,0,0.06)' }}>
            <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--accent-success)', boxShadow: isDark ? '0 0 10px #00C6AE' : '0 0 10px #3B82F6', animation: 'pulse-glow 2s infinite' }} />
            <span style={{ fontSize: '0.65rem', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.15em', color: isDark ? 'rgba(0,198,174,0.9)' : 'rgba(59,130,246,0.9)' }}>VAULT_ENGINE</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.75rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ color: isDark ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.4)' }}>NODES</span>
              <span style={{ color: 'var(--accent-success)', fontWeight: 600 }}>14,204</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ color: isDark ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.4)' }}>MANUAL SCANS</span>
              <span style={{ color: isDark ? '#00C6AE' : '#3B82F6', fontWeight: 600 }}>{manualScans}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px', paddingTop: '4px', borderTop: isDark ? '1px dotted rgba(255,255,255,0.1)' : '1px dotted rgba(0,0,0,0.1)' }}>
