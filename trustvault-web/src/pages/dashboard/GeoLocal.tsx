...
export default function GeoLocal() {
  ...
  const geoEvents = useMemo(() => {
    return alerts.map(alert => ({
      ...
      status: alert.status === 'resolved' || alert.status === 'closed' ? 'resolved' : alert.status === 'investigating' ? 'warning' : 'active'
    }));
  }, [alerts]);
  ...
  const filteredEvents = useMemo(() => {
    if (filter === 'all') return geoEvents;
    return geoEvents.filter(e => e.status === filter);
  }, [filter, geoEvents]);

  const activeCount = geoEvents.filter(e => e.status === 'active').length;
  const warningCount = geoEvents.filter(e => e.status === 'warning').length;

  const handleEventClick = (event: typeof geoEvents[0]) => {
  ...
  return (
    <div style={{ maxWidth: '1440px', margin: '0 auto' }}>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Active Nodes', value: geoEvents.length, icon: <Signal size={16} />, color: 'var(--brand-primary)' },
